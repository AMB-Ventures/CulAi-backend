var enquiry = require('../controllers/enquiry.controller');
...
module.exports = function (router) {
    ...
    router.get('/enquiries', [auth.verifyToken, auth.isSubAdmin], enquiry.getEnquiries);
    ...
    router.post('/:enquiryId/onboarding-data', upload.fields([{ name: 'menuList', maxCount: 1 }, { name: 'menuItems', maxCount: 1 }, { name: 'nda', maxCount: 1 }, { name: 'logo', maxCount: 1 }, { name: 'contract', maxCount: 1 }]), enquiry.createOnboardingData);
    router.get('/onboarding/:enquiryId', enquiry.getOnboardingDataById);
    router.get('/onboarding-data', enquiry.getAllOnboardingData);
    router.post('/:enquiryId', enquiry.approveOrRejectEnquiry);
    router.get('/onboarding/:enquiryId', enquiry.getOnboardingDataById);
    router.get('/onboarding-data', enquiry.getAllOnboardingData);
    router.get('/:enquiryId/user-status', enquiry.getUserStatus);
    router.post('/:enquiryId/approve-onboarding', enquiry.approveOrRejectOnboarding);
    router.patch('/:enquiryId/assign/:assignedTo', enquiry.assignEnquiry);
    router.get('/enquiry/:exportType/menu-excel', validator.params(querySchemaExport), enquiry.exportMenuXLSX);
    router.post('/enquiry/:_id', upload.fields([{ name: 'doc', maxCount: 1 }]), enquiry.uploadBankDetails);
    router.patch('/updateenquiry', enquiry.updateenquiry);
    router.patch('/enquiry/forgotpassword', enquiry.forgetPassword);
    router.patch('/enquiry/:enquiryId/reset-password', enquiry.resetPassword);
}