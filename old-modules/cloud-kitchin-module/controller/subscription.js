...
exports.updateSubscription = async (req, res, next) => {
    ...
    if (result) {
        ...
        if (sData) {
            ...
            const date = new Date();
            if (subsData.subscriptionFrequency === 'Monthly') var newDate = addDays(date, 30).toISOString();
            if (subsData.subscriptionFrequency === 'Half-yearly') var newDate = addDays(date, 180).toISOString();
            if (subsData.subscriptionFrequency === 'Yearly') var newDate = addDays(date, 360).toISOString();
            subsData.endDate = newDate;
            subsData.startDate = new Date().toISOString();
            subscriptionObject.subscription = subsData;
            let fData = await cloudKitchen.update({ kitchenId: kitchenId }, subscriptionObject);
