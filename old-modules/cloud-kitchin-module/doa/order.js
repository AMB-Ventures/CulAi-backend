...
orderSchema.statics = {
    ...
    getOrderById: async function (query) {
        let result = await this.find(query, {
            _id: 0,
            orderId: 1,
            kitchenId: 1,
    ...
    deleteManyTest: async function () {
        let result = await this.deleteMany();
        return result;
    },
    getStatisticsByStatus: async function (kitchenId) {
        let result = await this.aggregate([
            { '$match': { 'kitchenId': parseInt(kitchenId) } },
            { '$group': { '_id': '$orderStatus', 'count': { '$sum': 1 } } }]);
        return result.reduce((a, v) => ({ ...a, [v._id]: v.count }), {});
    },
}
