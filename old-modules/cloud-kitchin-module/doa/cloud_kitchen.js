...
cloud_kitchenSchema.statics = {
    ...
    getOne: async function (query) {
        let result = await this.findOne(query, {
            ...
            "merchantConfig.merchantLogo": 1,
            'schedule': 1
        }).populate('schedule.all').populate('schedule.sunday').populate('schedule.monday').populate('schedule.tuesday')
            .populate('schedule.wednesday').populate('schedule.thursday').populate('schedule.friday')
...
var cloudkitchenModel = mongoose.model('cloud_kitchen', cloud_kitchenSchema);
