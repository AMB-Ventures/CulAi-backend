...
module.exports = function (router) {
    router.post('/cloudkitchen', upload.single('kitchenLogo'), cloudKitchen.createCloudkitchen);
    ...
    router.put('/cloudkitchen/:kitchenId',upload.single('kitchenLogo'), cloudKitchen.updateCloudkitchen);
    router.get('/cloudkitchen/:exportType/export-document', [auth.verifyToken, auth.isSubAdmin], validator.params(querySchemaExport), cloudKitchen.exportDocument);
    router.patch('/cloudkitchen/:kitchenId/Publish-status', [auth.verifyToken, auth.isSubAdmin], cloudKitchen.updatePublish);
    router.get('/cloudkitchen/:kitchenId/merchantId/:merchantId', cloudKitchen.getCloudkitchenBymerchantId);
    router.get('/cloudkitchen/:kitchenId/merchants', cloudKitchen.getallmerchants);
    router.post('/cloudkitchen/:kitchenId/subscription', cloudKitchen.updateSubscription);
}