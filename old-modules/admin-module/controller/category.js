...
exports.exportDocument = async(req, res, next) => {
    ...
    try {
        ...
        tempResult = JSON.parse(tempResult.replace(/\b(?:categoryId|categoryName)\b/gi, matched => tempobj[matched]));
        // tempResult = JSON.parse(tempResult.split("categoryName").join("CuisineName"));
        // console.log("the data is " + tempResult)
        var filePath;
        const fileName = "category";
        const fields = ['CuisineName', 'CuisineId', 'description', "publish"];
        if (exportType === "csv") {
            filePath = await exportDoc.getCSV(tempResult, fields, fileName);
            ...
            filePath = await exportDoc.getXLSX(tempResult, fields, fileName);
            res.download(filePath, function(err) {
                if (err) {
                    console.log(err);
                }
